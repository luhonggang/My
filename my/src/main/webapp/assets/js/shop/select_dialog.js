...
function selectCategory(level, type, callback)
	{
		...
		d = dialog({
            ...
            title: '选择钢铁种类',
			url: '/category/select_category.htm?level='+level,
            width:'670',
			height:'500',
			data:param,
            fixed:true,
...
function selectMaterial(categoryCode, type, callback,flag)
{
	...
	d = dialog({
        ...
        title: '选择钢铁种类材质',
		url: '/material/select_material.htm?categoryCode='+categoryCode + "&flag=" + flag,
        width:'670',
		height:'500',
		data:param,
        fixed:true,
...
function selectSpec(categoryCode, type, callback, flag)
{
	...
	d = dialog({
        id: 'material_choose_iframe',
        title: '选择钢铁种类规格',
        url: '/spec/select_spece.htm?categoryCode='+categoryCode + "&flag=" + flag,
        width:'670',
		height:'500',
		data:param,
        fixed:true,
...
function selectWarehouse(callback)
{
	d = dialog({
		...
		title: '选择仓库',
		url: '/warehouse/select_warehouse.htm',
		width:'670',
		height:'410',
		fixed:'scrolling',
	}); 
	...
	d.addEventListener('close', function () {
		...
		//从仓库选择弹框 选择确定的返回数据
			...
			var spec={};
			spec["warehouseName"]=this.returnValue.name;
			spec["warehouseId"]=this.returnValue.id;
			spec["cityCode"]=this.returnValue.cityCode;
			callback(spec);
		}
	});
